...
- *dbt Tests:* dbt tests enforce data quality and integrity in the analytics layer. Generic schema tests validate uniqueness, non-null constraints, and referential relationships between facts and dimensions. [Custom tests](https://github.com/jfox1620/Portfolio/blob/main/Etsy%20Shop%20Data%20Pipeline/dbt%20Project/Tests/) enforce business logic rules, ensuring the transformed data remains consistent with expected system behavior. Tests are executed as part of the automated dbt build process to validate the pipeline continuously.
...
- `fact_orders`
  ...
  + Contains both revenue and fulfillment cost metrics
- `fact_transactions`
...
- Changing to incremental materialization of fact tables for large-scale growth
